...
- **Live post title (Q13): "Don't let your AI decide which rules to follow"** (no trailing period; matches the frontmatter exactly). Picked over the research-recommended "AI rules drift. Shell scripts don't." after the searchability test. New slug: `dont-let-ai-decide`. (2026-05-21)
...
- **Plain-substantive titles over flashy-claim, and frame by domain.** A title can pass "claim not topic" and still be hollow (a claim _performed_, not stated). JM rejected three reframe-flourish title options on the company-AI-infrastructure post as "flashy title-y with no substance," and steered "this is basic company ops, don't focus on engineering." Two rules landed in `_guide.md` Title section: (1) when framings tie, say the plainer thing; flashy-hollow = marketing for the post rather than its actual claim; (2) frame the topic by what it is (ops vs eng) and ground in the plainest real-world analogy (Jira/CRM/CI shared install) before any novel coinage. The Jira analogy was JM's, and carried the final post. (2026-06-29, on `company-ai-infrastructure`)
